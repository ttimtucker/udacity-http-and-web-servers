...
def CheckURI(uri, timeout=5):
    ...
    (i.e. times out).
    '''
    print('CheckURI: checking: ', uri)
    try:
        r = requests.get(uri)
        print('CheckURI: requests.get() returned: ', r.status_code)
    except requests.RequestException as e:
        print('CheckURI: requests.RequestException')
        return False
...
class Shortener(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if name:
            ...
            print('do_GET: root path, send the form, memory = ', memory)
    ...
    def do_POST(self):
        ...
        print('do_POST: longuri = ', params.get('longuri','Null'))
        print('do_POST: shortname = ', params.get('shortname','Null'))
        # Check that the user submitted the form fields.
        if "longuri" not in params or "shortname" not in params:
            # 3. Serve a 400 error with a useful message.
            #    Delete the following line.
            print('do_POST: one or more missing params')
            self.send_response(400)
            self.send_header('Content-type', 'text/html')
            self.end_headers()
            # List the known associations in the form.
            self.wfile.write(form.format('Error 400: missing parameter').encode())
            return
        ...
        if CheckURI(longuri):
            ...
            print('do_POST: longuri is good.  memory = ', memory)
            ...
            self.end_headers()
            # List the known associations in the form.
            self.wfile.write(form.format('Error 404: invalid long URI').encode())
            return
